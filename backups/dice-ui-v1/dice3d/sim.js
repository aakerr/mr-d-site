...
function traySize(aspect) {
  const a = Math.min(2.1, Math.max(0.55, aspect));
  const base = 10.5;
...
export function createDiceSim({ container, audio }) {
  ...
  function frameCamera() {
    const radius = Math.hypot(trayW, trayD) * 0.42;
  ...
  // --- live dice ----------------------------------------------------------
  let dice = [];         // { type, data, mesh, body, restFrames, nudges, resolved, value, display, chip }
  let rolling = false;
  ...
  function removeDie(die) {
    ...
    if (die.chip) overlay.removeChild(die.chip);
  }
  ...
  function spawnDie(type, i, count) {
    ...
    return {
      ...
      restFrames: 0, nudges: 0, resolved: false, value: null, display: null, chip: null,
    };
  }
  ...
  function setHouse(house) {
    ...
    for (const die of dice) {
      if (die.chip) {
        const a = accentFor(die.type);
        die.chip.style.borderColor = a;
        die.chip.style.boxShadow = `0 0 14px ${a}88`;
      }
    }
    renderOnce();
  }
  ...
  function settle(now) {
    ...
    pauseAt = now + 900;
    audio?.sfx?.('thud');
    for (const die of dice) makeChip(die);
    positionChips();
    const results = dice.map((d) => ({ value: d.value, display: d.display }));
    if (settleResolve) { settleResolve(results); settleResolve = null; }
  }

  function makeChip(die) {
    const el = document.createElement('div');
    const accent = accentFor(die.type);
    el.textContent = die.display;
    el.style.cssText =
      `position:absolute;transform:translate(-50%,-100%);padding:.2rem .7rem;` +
      `border-radius:9999px;font-weight:800;font-size:1.4rem;line-height:1;` +
      `color:#fff;background:rgba(11,15,25,.9);border:2px solid ${accent};` +
      `box-shadow:0 0 14px ${accent}88;white-space:nowrap;` +
      `font-family:system-ui,sans-serif;transition:opacity .2s ease;`;
  ...
  function positionChips() {
    ...
    for (const die of dice) {
      if (!die.chip) continue;
      _v.set(die.body.position.x, die.body.position.y + die.radius + 0.6, die.body.position.z);
      _v.project(camera);
      const x = (_v.x * 0.5 + 0.5) * rect.width;
      const y = (-_v.y * 0.5 + 0.5) * rect.height;
      die.chip.style.left = `${x}px`;
      die.chip.style.top = `${y}px`;
    }
  }
  ...
  function frame(now) {
    ...
    if (dice.some((d) => d.chip)) positionChips();
    renderer.render(scene, camera);
