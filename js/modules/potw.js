...
function createMap3d() {
  ...
  // HYBRID, and the cost is known and accepted. Labels are the load here, not
  // imagery — measured cold at Giza: 526 requests and ~7.1s to settle against
  // satellite's 357 and ~4s, on byte-identical tiles. That is real latency on
  // school wifi. It is paid anyway, on the owner's call, because a 3D map of
  // the ancient world carrying no place names reads as an anonymous lump of
  // terrain: the names ARE the lesson. Maps offers no way to keep the
  // geographic labels and drop the commercial ones, so "Great Pyramid of Giza"
  // arrives with KFC and Carrefour beside it — all-or-nothing, and some text
  // beats no text in front of a class. The atmosphere glow stays on too; it is
  // what makes the start of the flight look like space, and it is worth ~1.5s.
