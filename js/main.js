...
maps.async = true;
maps.src = `https://maps.googleapis.com/maps/api/js?key=${CONFIG.MAPS_API_KEY}&v=beta&libraries=maps3d`;
document.head.appendChild(maps);

const ctx = { store, registry, audio };
registry.init(ctx);

[dashboard, houses, potw, dice, battle, shop, admin].forEach((m) => registry.register(m));
